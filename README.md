...
三份有效回答是当前校准上限，而不是渠道请求次数上限：每份回答本身包含约 300 个整数，统一指纹库对 1、2、3 份回答分别做过概率校准。当前库的交叉验证准确率约为 95.5%、99.6%、100%；页面会在第 1、2、3 份有效回答到达时分别更新结果。继续增加有效回答会参与分数平均，但仍套用三份回答的概率校准，因此不再多测；只有失败、拒答或截断才会补试，累计最多尝试 6 次。进度区初始显示 3 列，每次补试动态增加 1 列并重新均分单行宽度。
